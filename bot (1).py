...
# -*- coding: utf-8 -*-
"""
Private Telegram Bot: Download TikTok/YouTube videos <= 50MB
- Restricts usage to ALLOWED_IDS
- Uses yt-dlp for downloading; ffmpeg for re-encoding to fit size
- Sends as video (not as document) when possible
"""
import os
import re
import math
...
if not BOT_TOKEN:
    raise SystemExit("BOT_TOKEN not set. Put it in .env")
if not ALLOWED_IDS:
    print("WARNING: ALLOWED_IDS is empty. Bot will reject everyone until you add your Telegram user IDs.")

# Minimal rate limit (seconds per download per user)
...
def compute_audio_bitrate(duration: float, target_bytes: int, ceiling_kbps: int = 320) -> int:
    ...
    # leave small container overhead (~16kbps)
...
def yt_dlp_base(url: str) -> str:
    """
    Build base yt-dlp command with extractor args for TikTok to prefer no-watermark & HD.
    """
    base = 'yt-dlp --no-call-home --no-warnings'
    if is_tiktok(url):
        # Prefer HD and nowatermark endpoints when available
...
async def probe_info(url: str) -> Dict[str, Any]:
    ...
    # If it's a playlist or multi-format, pick first entry
...
def pick_direct_under_limit(formats: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    # Prefer mp4 with both audio+video and filesize known <= limit
    candidates = []
    for f in formats or []:
